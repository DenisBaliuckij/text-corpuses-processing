...
with DAG(
    dag_id="download_russian_literature_classic",
    schedule="@continuous",
    ...
    @task()
    def download_russian_literature_classic():
        ...
        def fetch_page(criterion, page, proxy):
            urls, has_more = search_pdfs(
                ...
                proxies=None, tag='russian_literature_classic',
            )
            max_results = criterion.get('max_results', 500)
            has_more = has_more and (page * ROWS) < max_results
            return urls, has_more

        run_search(service_id=17, source='russian_literature_classic', adapter_fn=fetch_page, use_proxy=False)

    download_russian_literature_classic()
