...
with DAG(
    dag_id="download_english_science",
    schedule="@continuous",
    ...
    def download_english_science():
        ...
        def fetch_page(criterion, page, proxy):
            urls, has_more = search_pdfs(
                ...
                proxies=None, tag='english_science',
            )
            max_results = criterion.get('max_results', 5000)
            has_more = has_more and (page * ROWS) < max_results
            return urls, has_more

        run_search(service_id=21, source='english_science', adapter_fn=fetch_page, use_proxy=False)

    download_english_science()
