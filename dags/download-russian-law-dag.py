    ...
    def download_russian_law():
        ...
        def fetch_page(criterion, page, proxy):
            urls, has_more = search_pdfs(
                ...
                proxies=None, tag='russian_law',
            )
            max_results = criterion.get('max_results', 700)
            has_more = has_more and (page * ROWS) < max_results
            return urls, has_more

        run_search(service_id=19, source='russian_law', adapter_fn=fetch_page, use_proxy=False)

    download_russian_law()
