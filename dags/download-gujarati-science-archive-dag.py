...
with DAG(
    dag_id="download_gujarati_science_archive",
    schedule="@continuous",
    ...
    @task()
    def download_gujarati_science_archive():
        ...
        def fetch_page(criterion, page, proxy):
            urls, has_more = search_pdfs(
                ...
                proxies=None, tag='gujarati_science_natural',
            )
            max_results = criterion.get('max_results', 1000)
            has_more = has_more and (page * ROWS) < max_results
            return urls, has_more

        run_search(service_id=11, source='gujarati_science_archive', adapter_fn=fetch_page, use_proxy=False)

    download_gujarati_science_archive()
