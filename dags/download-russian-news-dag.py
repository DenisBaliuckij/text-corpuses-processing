    ...
    def download_russian_news():
        ...
        def fetch_page(criterion, page, proxy):
            urls, has_more = search_pdfs(
                ...
                proxies=None, tag='russian_news',
            )
            max_results = criterion.get('max_results', 2000)
            has_more = has_more and (page * ROWS) < max_results
            return urls, has_more

        run_search(service_id=18, source='russian_news', adapter_fn=fetch_page, use_proxy=False)

    download_russian_news()
