...
with DAG(
    dag_id="download_gujarati_official",
    schedule="@continuous",
    ...
    @task()
    def download_gujarati_official():
        ...
        def fetch_page(criterion, page, proxy):
            urls, has_more = search_pdfs(
                ...
                proxies=None, tag='gujarati_official',
            )
            max_results = criterion.get('max_results', 5000)
            has_more = has_more and (page * ROWS) < max_results
            return urls, has_more

        run_search(service_id=13, source='gujarati_official', adapter_fn=fetch_page, use_proxy=False)

    download_gujarati_official()
